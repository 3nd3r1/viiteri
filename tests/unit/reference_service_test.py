...
class TestReferenceService(unittest.TestCase):
    ...
    def test_create_reference(self):
        ...
        self.reference_service.create_reference("inproceedings", cite_key="johinp",
                                                author="John Doe",
                                                title="An Analysis of Example",
                                                booktitle="Sample Text",
                                                year="2002", editor="Ex Ample")
    ...
    def test_get_filtered_references(self):
        """ Test that get_filtered_references returns the correct references """
    ...
    def test_get_reference_by_doi(self):
        ...
        reference_example_1 = self.reference_service.get_reference_by_doi(
            "https://dl.acm.org/doi/10.1145/2380552.2380613")
        reference_example_2 = self.reference_service.get_reference_by_doi(
        ...
        self.assertEqual(
            reference_example_2['author'], "Held, Leonhard and Ott, Manuela")
    ...
    def test_get_reference_by_doi_raises_error_with_invalid_doi(self):
        ...
        with self.assertRaises(ValueError) as error:
            self.reference_service.get_reference_by_doi(
                "https://doi.org/3.1415926535")
        self.assertEqual(str(error.exception), "Invalid DOI")
        with self.assertRaises(ValueError) as error:
            self.reference_service.get_reference_by_doi(
                "10.114/annurev-statistics-031017-100307")
        self.assertEqual(str(error.exception), "Invalid DOI")

    def test_get_reference_by_doi_raises_error_with_unsupported_reference_type(self):
        """ 
          Test that get_reference_by_doi raises error with
          valid DOI and unsupported reference type 
        """
        with self.assertRaises(ValueError) as error:
            self.reference_service.get_reference_by_doi(
                "10.17077/etd.g638o927")
        self.assertEqual(str(error.exception),
                         "Unsupported reference type phdthesis")
        with self.assertRaises(ValueError) as error:
            self.reference_service.get_reference_by_doi(
                "https://doi.org/10.1007/0-387-21645-6_7")
        self.assertEqual(str(error.exception),
                         "Unsupported reference type inbook")

    @patch('viiteri.services.reference_service.get')
    def test_get_reference_by_doi_failed_to_find_reference(self, mock_get):
        """ Test get_reference_by_doi when reference is not found """
    ...
    def test_get_reference_by_doi_timeout(self, mock_get):
        """ Test get_reference_by_doi when request times out """
